...
const packTitlesUrl   = `https://sheets.googleapis.com/v4/spreadsheets/${packsSheetId}/values/${encodeURIComponent('Pack Titles!A2:A')}?alt=json&key=${apiKey}`;
const varietyPacksUrl = `https://sheets.googleapis.com/v4/spreadsheets/${packsSheetId}/values/${encodeURIComponent('Variety Packs!A2:C1000')}?alt=json&key=${apiKey}`;
...
// ———————————————————————————————————————————————
// LOCATION LOGIC (YOUR REALITY)
// Aisle 1 FIRST
// Aisle 1 = pick BOTH sides (Wall + Island Right)
// Aisle 2 = pick ONE side (Island Left)
// Skid number ignored, first letter used.
// ———————————————————————————————————————————————
function guessLocation(title) {
  ...
  // Optional overrides ONLY for ambiguous letters (C/T/etc)
  // side: 'WALL' | 'ISLAND_RIGHT' | 'ISLAND_LEFT'
  const OVERRIDES = [
  ...
  // AISLE 1 BAYS: you can grab both sides while standing in the aisle.
  // Fill these bays to match your labels. Repeating letters is OK.
  const AISLE1_BAYS = [
  ...
  const a1WallHits  = hitsA1(letter, 'WALL');
  const a1RightHits = hitsA1(letter, 'ISLAND_RIGHT');
  const a2 = bayA2(letter);

  let aisle = 9;
  let bay = 999;
  let side = 'UNKNOWN';

  // Force when ambiguous
  if (forcedSide === 'WALL' || forcedSide === 'ISLAND_RIGHT') {
    ...
    if (a1WallHits.length || a1RightHits.length) {
      ...
      // If letter appears on both sides (ex C), default wall unless overridden
  ...
  // Sort: Aisle 1 first, then Aisle 2
  // Within Aisle 1: bay order (so you pick both sides as you walk)
  // Within bay: WALL then ISLAND_RIGHT
  const sideRank =
    side === 'WALL' ? 1 :
    side === 'ISLAND_RIGHT' ? 2 :
    side === 'ISLAND_LEFT' ? 1 :
...
// ———————————————————————————————————————————————
// IMAGE LOOKUP LOAD (shows real error)
// ———————————————————————————————————————————————
async function loadImageLookup() {
  try {
    const res = await fetch(imageLookupUrl);
    if (!res.ok) throw new Error(`ImageLookup fetch failed ${res.status}: ${await res.text()}`);
...
// ———————————————————————————————————————————————
// VARIETY PACK MAP LOAD (optional)
// ———————————————————————————————————————————————
async function loadVarietyPackMap() {
  try {
    const res = await fetch(varietyPacksUrl);
    if (!res.ok) throw new Error(`Variety Packs fetch failed ${res.status}: ${await res.text()}`);
    ...
    console.warn('Variety pack map failed (continuing without pack explosion).', e);
    packMap = new Map();
  }
}

// ———————————————————————————————————————————————
// DATA: LOAD + PARSE ORDERS (FIXED GROUPING)
// Orders count = unique orderId (not rows)
// ———————————————————————————————————————————————
async function loadOrders() {
  try {
    ...
    console.error(err);
    renderStartError(`Failed to load orders: ${err.message || err}`);
  }
}

// ———————————————————————————————————————————————
// SAME PICK LOGIC USED BY BOTH MODES
// ———————————————————————————————————————————————
function expandAndMerge(items) {
  const map = new Map();
  ...
  for (const it of items) {
    ...
    // If it's a pack, explode it (assumes Variety Packs sheet is one row per can)
...
function buildPickQueue(orderList) {
  const allItems = [];
  for (const o of orderList) allItems.push(...(o.items || []));
  return expandAndMerge(allItems);
}

// ———————————————————————————————————————————————
// START VIEW (FIXED: shows real order count)
// ———————————————————————————————————————————————
...
function renderPackOrder() {
  ...
  // ✅ Per-order pick list using SAME logic
  const pickSorted = expandAndMerge(o.items || []);
...
// ———————————————————————————————————————————————
// PACK PICKER (LAZY-LOAD) — optional helper panel
// ———————————————————————————————————————————————
