...
function parseLocCode(locCode){
  const raw = safe(locCode).toUpperCase();
  if(!raw) return null;

  /*
    FINAL DESIGNATED WAREHOUSE PICK PATH
  ...
  */
  ...
  // Accept clean format like A-01 or A-01.5.
  // Also tolerates A1 / A-1 / A1.5 and normalizes the sort.
  const m = raw.match(/^([ABCD])[-\s]?(\d{1,2})(?:\.(5))?$/);
  ...
  const half = m[3] === '5' ? 0.5 : 0;

  if(!Number.isFinite(n) || n < 1 || n > 12){
    return {
      zone: aisle,
      idx: Number.isFinite(n) ? n : 999,
      side: '',
      sortKey: [9999, 999, half],
      normalizedLoc: raw
    };
  }
  ...
  // Half-location comes directly after the matching whole location.
  // Example: A-01, A-01.5, then B-01, B-01.5.
  const halfOrder = half === 0.5 ? 1 : 0;

  let major;

  if(aisle === 'A' || aisle === 'B'){
    // A/B walks forward from 1 -> 12.
    // For each number, pick A then B.
    major = ((n - 1) * 4) + (sideOrder * 2) + halfOrder;
  }else{
    // C/D walks backward from 12 -> 1.
    // For each number, pick C then D.
    major = ((12 - n) * 4) + (sideOrder * 2) + halfOrder + 100;
  }
  ...
  return {
    ...
    side: '',
    sortKey: [major, 0, 0],
    normalizedLoc: `${aisle}-${String(n).padStart(2, '0')}${half === 0.5 ? '.5' : ''}`
  };
}
